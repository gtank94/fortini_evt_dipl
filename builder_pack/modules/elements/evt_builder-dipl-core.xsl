...
<xsl:stylesheet xpath-default-namespace="http://www.tei-c.org/ns/1.0"
	...
	<xsl:template match="tei:app" mode="dipl" priority="3">
		<xsl:element name="span">
			...
			<xsl:apply-templates select="tei:rdg" mode="#current"/>
		</xsl:element>
	</xsl:template>
	
	
	<xsl:template match="tei:rdg" mode="dipl">
		<xsl:element name="span">
			<xsl:call-template name="dataAttributesFromAttributes"/>
			<xsl:attribute name="class" select="$ed_name1, name()" separator="-"
	...
	<xsl:template match="tei:lem" mode="dipl">
		<xsl:element name="span">
			<xsl:call-template name="dataAttributesFromAttributes"/>
			<xsl:attribute name="class" select="$ed_name1, name()" separator="-"
			/>(<xsl:apply-templates mode="#current"/>)</xsl:element>
	...
	-->
	<xsl:template
		match="tei:sic | tei:damage |  tei:am | tei:orig | tei:reg | tei:abbr | tei:expan"
	...
	<xsl:template match="tei:supplied" mode="dipl">
		<xsl:element name="span">
			...
			<xsl:call-template name="dataAttributesFromAttributes"/>
			<xsl:apply-templates  mode="#current"/><!--select="normalize-space()"-->
		</xsl:element>
	</xsl:template>
